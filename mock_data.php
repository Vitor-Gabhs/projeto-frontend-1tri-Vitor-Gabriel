...
if (php_sapi_name() === 'cli') {
    ...
    foreach (MockData::pedidosDetalhados() as $p) {
        printf("  Pedido #%d | Cliente: %-15s | Status: %-20s | Total: R$ %.2f\n",
            $p['id'], $p['cliente'], $p['status_nome'], $p['valor_total']);
    }

    echo "\n=== Itens do Pedido #2 ===\n";
    foreach (MockData::itensPedido(2) as $item) {
        printf("  %-10s  qtd: %d  unit: R$ %.2f  subtotal: R$ %.2f\n",
            $item['peca_nome'], $item['quantidade'], $item['preco_unit'], $item['subtotal']);
    }
